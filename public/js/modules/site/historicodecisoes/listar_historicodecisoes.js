app.controller("Listar_historicodecisoesController", function($scope, $http, $timeout){
    var $ = jQuery;
    var ng = $scope;
    ...
    ng.salvar = function(){
        if(validar("frm-historicodecisoes")){
            ng.historicodecisoe.cursos_id = ng.id;
            if (ng.arquivos != null) {
            ...
            $.ajax({
                ...
                success:function(json){
                    if(json.result){
                        ...
                        ng.historicodecisoe.id=json.id;
                    }else{
        ...
        if(json.result)
        setTimeout(function () {window.location.replace(base_url+'/instituicoes/ver_historicodecisoes/'+ng.curso.id);}, 3000);
        }
    ...
    ng.incluir = function(){
        ng.historicodecisoe = {};
        ng.historicodecisoe.cursos_id = cursos_id;
    ...
    ng.listar_historicodecisoes = function(id){ //função que recebe os dados dos cursos enviados pela função listar_cursos_json do arquvio /modules/site/controllers/cursos.php.php
        cursos_id = id;
        ng.historicodecisoe = {};
        ng.historicodecisoe.cursos_id = id;
        $http.get(base_url+'/historicodecisoes/listar_historicodecisoes_json/'+id).success(function(data){
            ng.historicodecisoes = data;
            console.log(ng.historicodecisoe.data_deferimento);
        });
    }
    
    verhistoricodecisoes = function(id){
        
        $http.get(base_url+'/historicodecisoes/carregar/'+id).success(function(data){
            ng.historicodecisoe = data;
    ...
    ng.init = function(id){
        ...
        $http.get(base_url+'/historicodecisoes/ver_nomeinstituicao/'+ng.id).success(function(data){
            ng.historicodec = data[0];
            
        });
        $http.get(base_url+'/historicodecisoes/ver_nomecurso/'+ng.id).success(function(data){
            ng.historico = data[0];
        ...
        ng.listar_historicodecisoes(id);
        getMask();
        //ng.reload();
        ativaUpload();
