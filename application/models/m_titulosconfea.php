<?php

class M_titulosconfea extends CI_Model {

    public function salvar($data) {
        if (isset($data['$$hashKey'])) {
            unset($data['$$hashKey']);
        }
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $result = $this->db->update('titulosconfea', $data);
        }else{
            $result = $this->db->insert('titulosconfea', $data);
        }
        return array('result' => $result, 'message' => $result ? 'Registro gravado' : 'Erro ao gravar o registro', 'error' => $this->db->_error_message());
    }

    public function excluir($id) {
        $this->db->where('id', $id);
        $result = $this->db->delete('titulosconfea');
        return array('result' => $result, 'message' => $result ? 'Registro excluído' : 'Erro ao excluir o registro', 'error' => $this->db->_error_message());
    }

    public function total() {
        $this->db->select('count(id) as total');
        $this->db->from('titulosconfea');
        $query = $this->db->get();
        return $query->result();
    }

    public function lista($limite, $inicio, $coluna, $ordem) { 
        $this->db->select("ti.*");
        $this->db->from("titulosconfea ti");
        $this->db->limit($limite, $inicio);
        $this->db->order_by($ordem, $coluna);
        $query = $this->db->get();
        return is_object($query)?$query->result():array();
    }

    public function carregar($id) { 
        $this->db->select("ti.*");
        $this->db->from("titulosconfea ti");
        $this->db->where("ti.id = ",$id);
        $query = $this->db->get();
        return $query->result();
    }
    
    public function pesquisa($termo) {
        $this->db->select("id, titulo");
        $this->db->from("titulosconfea");
        $this->db->where("titulosconfea.titulo like '%$termo%' ORDER BY titulo ASC");
        $query = $this->db->get();
        return is_object($query)?$query->result():array();
    }
}